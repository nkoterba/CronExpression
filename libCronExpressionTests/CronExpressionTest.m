...
{
    // '2010-09-10 12:00:00'
    NSString * cronExpr = @"1 2-4 * 4,5,6 *___/3 2012";
    ...
    STAssertTrue([[expr getExpression:YEAR] isEqualToString:@"2012"], @"YEAR expression not equal");
...
-(void)testGetNextRunDate
{
    ...
    // Test annually
    CronExpression * expr = [CronExpression factory:@"@annually" :nil];
    NSDate *date = [expr getNextRunDate:[NSDate date] :0];
    ...
    NSDateComponents * add1Year = [[NSDateComponents alloc] init];
    add1Year.year = 1;
    NSDate *nextDate = [calendar dateByAddingComponents:add1Year toDate:[calendar dateFromComponents:components] options:0];
    
    STAssertEquals([date timeIntervalSince1970], [nextDate timeIntervalSince1970], @"Next date for annually is INVALID");

    // Test yearly
    expr = [CronExpression factory:@"@yearly" :nil];
    date = [expr getNextRunDate:[NSDate date] :0];
    
    STAssertEquals([date timeIntervalSince1970], [nextDate timeIntervalSince1970], @"Next date for yearly is INVALID");
    
    // Test monthly
    expr = [CronExpression factory:@"@monthly" :nil];
    date = [expr getNextRunDate:[NSDate date] :0];
    ...
    components.year = [calendar components:NSYearCalendarUnit fromDate:[NSDate date]].year;
    NSDateComponents * add1Month = [[NSDateComponents alloc] init];
    add1Month.month = 1;
    nextDate = [calendar dateByAddingComponents:add1Month toDate:[calendar dateFromComponents:components] options:0];
    
    STAssertEquals([date timeIntervalSince1970], [nextDate timeIntervalSince1970], @"Next date for monthly is INVALID");

    // Test weekly
//    expr = [CronExpression factory:@"@weekly" :nil];
//    date = [expr getNextRunDate:[NSDate date] :0];
//    
